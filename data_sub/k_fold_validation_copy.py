...
def make_valid(num_split, fromdata2="/opt/ml/detection/dataset/train.json",
fromdata="/opt/ml/detection/dataset/refined_train.json",
    ...
    with open(fromdata2,'r') as f2:
        json_data2 = json.load(f2)



    file_data['info'] = json_data2['info']
    file_data['licenses'] = json_data2['licenses']
    ...
    df['new_id']=[0 for i in range(len(df['id']))]
    temp=-1
    imid=-1
    for d in df.iterrows():
        if d[1]['image_id']!=imid:
            temp+=1
            imid=d[1]['image_id']
        d[1]['new_id']=temp
    ...
    for i, (train_idx, valid_idx) in enumerate(splits):
        ...
        train_ids = train['new_id'].drop_duplicates().values
        valid_ids = valid['new_id'].drop_duplicates().values  

        for t in train_ids:
            # t=int(t)
            file_data['images'].append(json_data['images'][t])
        for t2 in train['id']:
            file_data['annotations'].append(json_data['annotations'][t2])


        with open(tofolder+'rtrain_fold'+str(i+1)+'.json','w',encoding="utf-8") as make_file:
        ...
        for v2 in valid['id']:
            val_data['annotations'].append(json_data['annotations'][v2])


        with open(tofolder+'rval_fold'+str(i+1)+'.json','w',encoding="utf-8") as val_file:
